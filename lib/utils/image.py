...
def get_test_seg_rgb_image(roidb, cur_frame, end_frame, config):
    """
    preprocess image and return processed roidb
    :param roidb: a list of roidb
    :return: list of img as in mxnet format
    ...
    for i in range(num_images):
        ...
        residual = read_test_res(prefix, im.shape, im_scale, config.TRAIN.KEY_FRAME_INTERVAL, pos_target)
        ...
        assert (len(nearby_roidb)-1) == data_cache.shape[0] - 1 == residual.shape[0], 'len(nearby_roidb) == mv.shape[0] == residual.shape[0]'
...
def read_test_res(prefix, im_shape, im_scale, num_interval, pos_target):
    res_addr = prefix.replace('/VID/', '/RES_5/')+'.res'
    h = im_shape[0]
    w = im_shape[1]
    for i in range(4):# num of pooling layers
        h = math.floor(0.5*(h - 1)) +1
    ...
    res = np.fromfile(res_addr, dtype=np.float16)
    assert res.shape[0]%(3*h*w)==0, 'res.shape[0]%(3*h*w)==0'
    if res.shape[0] > num_interval*2*h*w:
        res = res[:num_interval*3*h*w]
    res = res.reshape((pos_target, 3, h, w))
    res = np.pad(res, ((0, num_interval - pos_target), (0,0), (0,0), (0,0)), 'constant')
    return res
...
def get_seg_image(roidb, config):
    ...
    return processed_ims, processed_mv, processed_residual, processed_nearby_roidb
def get_seg_RGB_image(roidb, config):
    """
    preprocess image and return processed roidb
    :param roidb: a list of roidb
    :return: list of img as in mxnet format
