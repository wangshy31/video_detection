...
def train_net(args, ctx, pretrained, pretrained_mini, epoch, prefix, begin_epoch, end_epoch, lr, lr_step):
    ...
    # infer max shape
    max_data_shape = [('data', (config.TRAIN.BATCH_IMAGES*(1+config.TRAIN.KEY_FRAME_INTERVAL),
                                3, max([v[0] for v in config.SCALES]),
    ...
    if config.TRAIN.RESUME:
        ...
        arg_params_mini, aux_params_mini = load_param(pretrained_mini, epoch, convert=True)
        arg_params.update(arg_params_mini)
        aux_params.update(aux_params_mini)
...
def main():
    ...
    train_net(args, ctx, config.network.pretrained, config.network.pretrained_mini, config.network.pretrained_epoch, config.TRAIN.model_prefix,
